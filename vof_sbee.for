...
c    integrating average Superbee TVD method      
	...
	real fluxx(0:isize,0:isize),fluxy(0:isize,0:isize) 
...
c       Solving... 
     ...
     &       !                       *(gama(i+1,j)-gama(i,j)) 
			...
			if(abs(c0(i,j)-c0(i-1,j)).gt.1.0e-6) then
			  cita1=(c0(i-1,j)-c0(i-2,j))/(c0(i,j)-c0(i-1,j)) 
			else 
			 cita1=0 
              endif       
			d1=c0(i-1,j)+0.5*abs(u(i,j))*(1.0-dt/h*abs(u(i,j)))*
     1        (c0(i,j)-c0(i-1,j))*fi(cita1)				 
			if(abs(c0(i,j)-c0(i-1,j)).gt.1.0e-6) then
			  cita2=(c0(i+1,j)-c0(i,j))/(c0(i,j)-c0(i-1,j)) 
			else 
			 cita2=0 
              endif       
			d2=c0(i,j)+0.5*abs(u(i,j))*(1.0-dt/h*abs(u(i,j)))*
     1        (c0(i+1,j)-c0(i,j))*fi(cita2)
			fluxx(i,j)=max(sign(1.0,u(i,j)),0.0)*d1
     1		           +min(sign(1.0,u(i,j)),0.0)*d2	
			
          enddo 
        enddo 
		do i=1,isize-1 
          do j=1,isize-1
		  c1(i,j)=c0(i,j)+ dt/h*(fluxx(i+1,j)-fluxx(i,j))
		end do
		end do 
